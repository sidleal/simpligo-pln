...
func main() {
	...
	buffer := readFile("/home/sidleal/sid/usp/Essays/100k_essays_metricas01.csv")
	...
	for i, line := range lines {
		...
		bufferMap[tokens[0]] = line
	}

	buffer2 := readFile("/home/sidleal/sid/usp/Essays/100k_essays_metricas02.csv")
	...
	raw := readFile("/home/sidleal/sid/usp/Essays/100k_redacoes.json")

	matchContent := regEx.FindAllStringSubmatch(raw, -1)

	count := 0
	for i, match := range matchContent {
		// log.Println("---------------------------------------------------------")
		if len(match) < 3 {
		...
		if metrics, found := bufferMap[textID]; found {
			log.Println("encontrou!!!", textID, theme, i, count)
...
func main1() {
	...
	f, err := os.OpenFile("/home/sidleal/sid/usp/Essays/essays_metricas.csv", os.O_TRUNC|os.O_CREATE|os.O_WRONLY, 0644)
	if err != nil {
		log.Println("ERRO", err)
	}
	defer f.Close()

	raw := readFile("/home/sidleal/sid/usp/Essays/essays.tsv")
	...
	for i, line := range lines {
		...
		texto := fields[2] + "<br>" + fields[3] + "<br>" + fields[4]
